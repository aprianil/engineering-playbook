...
**Review in this order.** Diff-level issues first (they're fast to spot), then design problems (they save you from reviewing code that might get rewritten), then details.

1. **Walk the diff** -- read it line by line before anything else. Flag concrete issues that are visible in the diff itself: logic bugs, off-by-one errors, wrong variable names, incomplete changes (renamed in one place but not another), accidental deletions, sensitive data (keys, tokens, account IDs), dead code introduced, copy-paste errors, missing error handling on new code paths. These are mechanical -- no principles needed, just careful reading.
2. Zoom out -- read the PR description or spec. Does the approach make sense? If not, stop here and say so.
3. Review the main files first -- the biggest, most important changes. This is where design problems live.
4. Review the rest once the design is solid.
