...
**Delegate implementation; stay lead.** Tasks whose approach is fully decided by the spec (feature slices, fixes, tests for decided behavior, repetitive edits) go to the codex agent by default — it bills a separate quota bucket from the orchestrator and its sub-agents. When in doubt whether a task is Codex-able, it is; reserve the main session for integration, judgment calls, and diff review. Use `fast-worker` only for edits so small a Codex round trip costs more than it saves. Pass the task's spec section, acceptance criteria, and file list inline so delegates don't re-explore. Review every delegated diff before checkpointing: delegation moves token burn, not accountability. Unpinned sub-agents inherit the main model; pin the tier deliberately.

**Parallelize independent tasks.** When the task list marks tasks independent, fire them as parallel background Codex runs — separate worktrees, per the parallel-build rule — and review diffs as each lands. Don't babysit one delegate while others could be running; the build's wall-clock is the slowest slice, not the sum.
